...
All secrets live in a single config file. Edit `static/ui/config.js` (never commit real keys):
...
Forge reads this file directly at runtime to authenticate GitHub and Gemini requests. Rotate tokens by updating the file before redeploying.

## 4. Lint & Deploy to Forge
...
- **`CONFIG not found`** – Check that `static/ui/config.js` exists and exports a `CONFIG` object with both keys.
...
- **UI changes not appearing** – Make sure you saved the files under `static/ui/` before running `forge deploy`.
...
You now have CodeGraph Demo (Dev) deployed in Jira. Repeat steps 4–5 whenever you make UI or backend changes that need to go live.
